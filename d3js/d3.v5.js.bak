...
;(function (global, factory) {
  ...
  function brush$1(dim) {
    ...
    function started() {
      ...
      var that = this,
        ...
        mode =
          (keys && exports.event.metaKey ? (type = "overlay") : type) ===
          "selection"
            ? MODE_DRAG
        ...
        lockX,
        lockY,
        pointer = exports.event.touches
          ? toucher(exports.event.changedTouches[0].identifier)
          : mouse,
        point0 = pointer(that),
        point = point0,
  ...
  function formatLocale(locale) {
    var group =
        locale.grouping === undefined || locale.thousands === undefined
          ? identity$3
          : formatGroup(
              map$2.call(locale.grouping, Number),
              locale.thousands + ""
            ),
      currencyPrefix =
      ...
      numerals =
        locale.numerals === undefined
          ? identity$3
          : formatNumerals(map$2.call(locale.numerals, String)),
  ...
  function rotationIdentity(lambda, phi) {
    return [
      abs(lambda) > pi$3
        ? lambda + Math.round(-lambda / tau$3) * tau$3
        : lambda,
      phi,
    ]
  }
  ...
  function transformer$1() {
    ...
    scale.domain = function (_) {
      return arguments.length
        ? ((domain = map$3.call(_, number$2)),
          clamp === identity$6 || (clamp = clamper(domain)),
          rescale())
        : domain.slice()
    }
    ...
    scale.rangeRound = function (_) {
      return (
        (range = slice$5.call(_)), (interpolate = interpolateRound), rescale()
      )
    }

    scale.clamp = function (_) {
      return arguments.length
        ? ((clamp = _ ? clamper(domain) : identity$6), scale)
        : clamp !== identity$6
    }
  ...
  function voronoi() {
    ...
    function voronoi(data) {
      return new Diagram(
        data.map(function (d, i) {
          var s = [
            Math.round(x(d, i, data) / epsilon$4) * epsilon$4,
            Math.round(y(d, i, data) / epsilon$4) * epsilon$4,
          ]
          s.index = i
          s.data = d
          return s
    ...
    voronoi.x = function (_) {
      return arguments.length
        ? ((x = typeof _ === "function" ? _ : constant$c(+_)), voronoi)
        : x
    }

    voronoi.y = function (_) {
      return arguments.length
        ? ((y = typeof _ === "function" ? _ : constant$c(+_)), voronoi)
        : y
    }
  ...
  function defaultWheelDelta() {
    return (
      ...
      (exports.event.deltaMode === 1
        ? 0.05
        : exports.event.deltaMode
        ? 1
        : 0.002)
    )
  }
